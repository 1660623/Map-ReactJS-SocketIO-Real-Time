...
class App extends Component {
  ...
  changeData = data => {
    ...
    this.setState({ location: data,
      ...
      zoom: 18 });
    console.log("chuyennho", this.state.location)
  };

  // changeData = (data) => socket.emit("initial_data");
  ...
  componentDidMount() {
    ...
    //     });
    //   });
    // socket.on("anhyeuem" , data)
    // console.log(data)
    // socket.on("get_data", this.getData);
    socket.on("anhyeuem", this.changeData);
  }
  ...
  showMessageForm = () => {
    ...
    .then(location => {
      ...
      console.log("anh yeu em ", location)
